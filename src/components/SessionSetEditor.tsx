  ...
  return (
    <details id={`set-${set.id}`} className="group rounded border border-border bg-surface open:border-accent">
      ...
      <div className="flex flex-col gap-3 border-t border-border p-3">
        <form action={updateSetEntry.bind(null, set.id, sessionId)} className="grid gap-3 sm:grid-cols-2 lg:grid-cols-6 lg:items-start">
          <LiftWeightFields
            idPrefix={`set-${set.id}`}
            lifts={lifts}
          ...
          <button type="submit" className="rounded bg-accent px-3 py-1.5 text-sm font-semibold text-accent-foreground lg:mt-5 lg:h-[34px]">Save</button>
        </form>
